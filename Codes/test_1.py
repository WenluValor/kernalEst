...
def set_global(d_value: int, n_value: int,
                  bound_value: np.array, p_value: int, s_value: int):
    global d, n, Y0, Z, s, p, bound
    d = d_value
    n = n_value
    s = s_value
    p = p_value
...
if __name__ == '__main__':
    random.seed(15)
    np.random.seed(4)
    set_global(d_value=3, n_value=21 ** 3,
               bound_value=np.array([80, 120, 0.01, 0.05, 0.2, 1]), p_value=0, s_value=5)
    ...
    for j in range(time):
        ...
        for i in range(size):
            t[i] = torch.tensor(get_t())
            Y_test[i] = torch.tensor(dt_1.true_f(t[i]))
