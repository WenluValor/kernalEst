...
def generate_data(T_value: int, P0_value: int, q_value: int, d_value: int, n_value: int,
    ...
    Y0 = np.zeros([n])
    vec_epsilon = np.zeros([n, p + 1])
    for i in range(n):
        Y0[i] = avg_Y0(t=vec_t[i])
        vec_epsilon[i, 0] = Y0[i] - true_f0(t=vec_t[i])
    DF = pd.DataFrame(Y0)
    if b == -1:
        DF.to_csv('Y0.csv')
    else:
    ...
    for j in range(p):
        if j == 0:
            for i in range(n):
                t = vec_t[i]
                Z[i, j] = avg_Y1(t)
                vec_epsilon[i, j + 1] = Z[i, j] - partial_true_f(t=t, part_ind=j)
        elif j == 1:
            for i in range(n):
                t = vec_t[i]
                Z[i, j] = avg_Y2(t)
    ...
    if b == -1:
        ...
        DF.to_csv('MSE_outcome/' + str(b) + '_vec_epsilon.csv')
...
def generate_eps(T_value: int, P0_value: int, q_value: int, d_value: int, n_value: int,
                  bound: np.array, p_value: int, b: int):
    global T, P0, q, n, d, p
    T = T_value
    P0 = P0_value
...
if __name__ == '__main__':
    generate_eps(T_value=1, P0_value=100, q_value=5000, d_value=3, n_value=21**3,
                  bound=np.array([80, 120, 0.01, 0.05, 0.2, 1]), p_value=2, b=-1)
