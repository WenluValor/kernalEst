...
def generate_data(p_value: int, n_value: int, d_value: int, up: float, down: float, b: int):
    np.random.seed(b + 5)
    ...
    full_ind = list(range(x_data.shape[0]))
    ...
    for i in range(n):
        ...
        vec_epsilon[i, 0] = Y0[i] - Y_0(t=vec_t[i], ind=full_ind)
    ...
    for j in range(p):
        ...
        for i in range(n):
            ...
            vec_epsilon[i, j + 1] = Z[i, j] - partial_Y(t=vec_t[i], j=j + 1, ind=full_ind)
    DF = pd.DataFrame(Z)
    if b == -1:
        DF.to_csv('Z.csv')
    else:
