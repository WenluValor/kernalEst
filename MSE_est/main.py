...
def set_global(b: int, s_value: int, n_value: int, d_value, p_value: int):
    ...
    if b == -1:
        ...
        Y0 = np.array(pd.read_csv('MSE_outcome/' + str(b) + '_Y0.csv', index_col=0))[:, 0]
        Y0 = torch.from_numpy(Y0)

        Z = np.array(pd.read_csv('MSE_outcome/' + str(b) + '_Z.csv', index_col=0))
        Z = torch.from_numpy(Z)
...
def get_vec_c_torch(lmd, weight: np.array, b: int):
    global MAT_PSI, PART_PSI_List
    if b == -1:
        MAT_PSI = np.array(pd.read_csv('mat_PSI.csv', index_col=0))
        MAT_PSI = torch.from_numpy(MAT_PSI)
    else:
        MAT_PSI = np.array(pd.read_csv('MSE_outcome/' + str(b) + '_mat_PSI.csv', index_col=0))
        MAT_PSI = torch.from_numpy(MAT_PSI)
    ...
    for i in range(p):
        if b == -1:
            ...
            mat_PSI = np.array(pd.read_csv('MSE_outcome/' + str(b) + '_' + str(i) + 'part_mat_PSI.csv', index_col=0))
            PART_PSI_List[i] = torch.from_numpy(mat_PSI)
...
def create_mat(s, N, d, p, b=-1):
    ...
    mat_PSI = get_pdPSI(vec_t=b)
    mat_PSI = mat_PSI.detach().numpy()
    DF = pd.DataFrame(mat_PSI)
    if b == -1:
        DF.to_csv('mat_PSI.csv')
    else:
        DF.to_csv('MSE_outcome/' + str(b) + '_mat_PSI.csv')

    for i in range(p):
        mat_PSI = get_partial_pdPSI(i, vec_t=b)
        mat_PSI = mat_PSI.detach().numpy()
        DF = pd.DataFrame(mat_PSI)
        if b == -1:
            DF.to_csv(str(i) + 'part_mat_PSI.csv')
        else:
            DF.to_csv('MSE_outcome/' + str(b) + '_' + str(i) + 'part_mat_PSI.csv')
    return
...
def hat_fn(X_test, vec_c):
    if not isinstance(X_test, int):
        ...
        if X_test == -1:
            ...
            mat_PSI = np.array(pd.read_csv('MSE_outcome/' + str(X_test) + '_mat_PSI.csv', index_col=0))
            mat_PSI = torch.from_numpy(mat_PSI)
...
def hat_partial_fn(X_test, vec_c, part_ind):
    if not isinstance(X_test, int):
        ...
        if X_test == -1:
            ...
            mat_PSI = np.array(pd.read_csv('MSE_outcome/' + str(X_test) + '_'
                                           + str(part_ind) + 'part_mat_PSI.csv', index_col=0))
            mat_PSI = torch.from_numpy(mat_PSI)
...
def get_pdPSI(vec_t=-1):
    if isinstance(vec_t, int):
        if vec_t == -1:
            ...
            vec_t = np.array(pd.read_csv('MSE_outcome/' + str(vec_t) + '_0vec_t.csv', index_col=0))
            vec_t = torch.from_numpy(vec_t)
...
def get_partial_pdPSI(part_ind, vec_t=-1):
    if isinstance(vec_t, int):
        if vec_t == -1:
            ...
            vec_t = np.array(pd.read_csv('MSE_outcome/' + str(vec_t) + '_'
                                         + str(part_ind + 1) + 'vec_t.csv', index_col=0))
            vec_t = torch.from_numpy(vec_t)
...
def boots_compute(N: int, s: int, d: int, p: int, b=-1, use_lmd=False):
    ...
    if use_lmd:
        ...
        if b == -1:
            ...
            DF.to_csv('MSE_outcome/' + str(b) + '_lmd.csv')
    else:
        lmd = torch.tensor([0])
    ...
    if b == -1:
        ...
        DF.to_csv('MSE_outcome/' + str(b) + '_torch_C.csv')
        os.remove('MSE_outcome/' + str(b) + '_mat_PSI.csv')
        os.remove('MSE_outcome/' + str(b) + '_Y0.csv')
        os.remove('MSE_outcome/' + str(b) + '_Z.csv')
        os.remove('MSE_outcome/' + str(b) + '_0vec_t.csv')
        for i in range(p):
            os.remove('MSE_outcome/' + str(b) + '_' + str(i) + 'part_mat_PSI.csv')
            os.remove('MSE_outcome/' + str(b) + '_' + str(i + 1) +'vec_t.csv')
    return


def gcv_func(lmd, b: int, is_scipy=False):
    if is_scipy:
        lmd = torch.tensor(lmd)

    if b == -1:
        vec_x = np.array(pd.read_csv('0vec_t.csv', index_col=0))
    else:
        vec_x = np.array(pd.read_csv('MSE_outcome/' + str(b) + '_0vec_t.csv', index_col=0))
    vec_x = torch.from_numpy(vec_x)
    for i in range(p):
        if b == -1:
            tmp = np.array(pd.read_csv(str(i + 1) + 'vec_t.csv', index_col=0))
        else:
            tmp = np.array(pd.read_csv('MSE_outcome/' + str(b) + '_'
                                       + str(i + 1) + 'vec_t.csv', index_col=0))
