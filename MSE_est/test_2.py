...
def set_global(b: int, c_value: np.array, s_value: int, n_value: int, p_value: int):
    global d, n, Y0, Z, s, p, Y0, Z, w_js, b_js
...
if __name__ == '__main__':
    ...
    for j in range(-1, time - 1):
        set_global(b=j, c_value=np.array([1, 0.8, 0.7, 0.6]), s_value=5, n_value=1, p_value=1)
        ...
        for ind_i in range(10):
            t = np.random.uniform(0.5, 1.5, size=(batch, d))
            test = np.insert(t, d - 1, 1, axis=1)
            Y_test = torch.tensor(dt_2.true_f(c, test))
