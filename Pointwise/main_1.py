...
def set_global(T_value: int, P0_value: int, q_value: int, d_value: int, n_value: int,
    ...
    knl_1.generate_wb(d_value=d, s_value=s)
...
def get_gcv_lmd():
    device = torch.device('cpu')
    # create_mat()
    lmd = gcv_solve(tol=1e-7, lr=1e-3, device=device)
    return lmd
...
if __name__ == '__main__':
    random.seed(15)
    np.random.seed(4)
    set_global(T_value=1, P0_value=100, q_value=1000, d_value=3, n_value=7 ** 3,
    ...
    for j in range(time):
        ...
        for i in range(size):
            ...
            Y_test[i] = torch.tensor(dt_1.true_f0(t[i]))
