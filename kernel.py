...
def opt_bandwidth(b):
    ...
    if b == -1:
        ...
        Y = np.array(pd.read_csv('outcome/' + str(b) + '_Y0.csv', index_col=0))
    ...
    # param_dist = {"kernel": [Matern(l1, nu=2.5)
                             # for l1 in np.logspace(-2, 2, 1000)]}
    ...
    if b == -1:
        ...
        DF.to_csv('outcome/' + str(b) + '_tau.csv')
    return
...
def generate_wb(d_value: int, s_value: int, b=-1):
    ...
    if b == -1:
        ...
        tau = np.array(pd.read_csv('outcome/' + str(b) + '_tau.csv', index_col=0))
    ...
    if b == -1:
        ...
        DF = pd.DataFrame(w_js)
        DF.to_csv('outcome/' + str(b) + '_w_js.csv')
        DF = pd.DataFrame(b_js)
        DF.to_csv('outcome/' + str(b) + '_b_js.csv')
    return
