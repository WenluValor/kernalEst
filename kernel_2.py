...
def opt_bandwith(index: int, file_index: int, fold=5):
    # index = 0, 1, 2
    minimum = minimize_scalar(func, bounds=(0, 10), method='bounded', args=(index, file_index, fold))
    return minimum.x

def func(bd, index: int, file_index: int, fold):
    file_name = str(file_index) + 'vec_t.csv'
...
def generate_wb(d: int, s: int, low, high):
    ...
    for i in range(d):
        ...
        for j in range(d):
            # j is coord_index
            A = opt_bandwith(j, i)
            w[j] = sample_p(s, low, high, A)
            b[j] = np.random.uniform(0, 2 * math.pi, size=s)

        DF = pd.DataFrame(w)
        DF.to_csv(str(i) + 'w_js.csv')
        DF = pd.DataFrame(b)
        DF.to_csv(str(i) + 'b_js.csv')
